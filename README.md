...
- Load a Pokemon Showdown replay from a replay URL or replay ID, with input validation and readable error messages. Smogtours ids are normalized to their real formats (`smogtours-gen3ou-…` → `gen3ou`), and private replay links load from the full `-…pw` password link — the password is kept for the fetch and kept out of the inferred format. A replay whose JSON route comes back unusable is retried on the replay server's `.log` route before the loader gives up.
...
- `npm run test:e2e` passes with 65 browser tests (the replay JSON and the Showdown embed script are served from fixtures/cache, so the suite is CDN-independent).
...
- silently truncate inside the variation, confirm before replacing it from the main line
...
5. a per-turn seed search that aligns the simulator's RNG (crits, misses, secondary effects, faints) with what the real game's protocol actually recorded, so replayed positions no longer carry phantom rolls the real game disproves
