...
- [`src/lib/eval/`](./src/lib/eval/) contains the evaluation engine: a sim forward model, a static positional eval with a boost-aware matchup threat term, maximin search with iterative deepening, a DUCT-MCTS mode, a pure async orchestrator fanned out over a worker pool, played-action parsing (singles and per-slot doubles), per-turn analysis with verdicts, natural-language summaries, and the game report.
