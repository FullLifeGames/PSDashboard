...
- [`src/lib/branch-engine.ts`](./src/lib/branch-engine.ts) is the facade of the reconstruction engine. [`src/lib/branch/`](./src/lib/branch/) holds its types, team ordering, protocol choice extraction, boundary corrections, log sync, state builders, choice execution, and the staged reconstruction pipeline over one session.
- [`src/lib/protocol-parser.ts`](./src/lib/protocol-parser.ts) converts replay protocol logs into turn snapshots; [`src/lib/protocol/`](./src/lib/protocol/) holds the scan state and the per-tag handlers.
- [`src/lib/opponent-inferrer.ts`](./src/lib/opponent-inferrer.ts) extracts revealed team information from the replay log; [`src/lib/inference/`](./src/lib/inference/) holds the lookup helpers, the scan state, and the per-line handlers.
- [`src/lib/team-builder.ts`](./src/lib/team-builder.ts) builds simulator teams from replay evidence, pasted teams, and usage-stat guesses; [`src/lib/team/set-resolvers.ts`](./src/lib/team/set-resolvers.ts) resolves each set field with its precedence chain.
- [`src/lib/smogon-stats.ts`](./src/lib/smogon-stats.ts) fetches and normalizes Smogon monthly usage probabilities; [`src/lib/smogon/`](./src/lib/smogon/) holds the usage types and the payload parsers.
