...
- `npm run test:e2e` passes with 68 browser tests (the replay JSON and the Showdown embed script are served from fixtures/cache, so the suite is CDN-independent).
...
6. Choose a move or switch for both sides. The action taken in the real game carries a "played" badge, and the "Advanced" toggle adds a free-choice dropdown plus "What if it had …". Then execute: the simulator rebuilds at that position and your variation opens (chess-style; there is no separate branch mode). The timeline slider marks the variation's span in gold.
