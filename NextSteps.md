...
- [ ] **T85 · Zug-Knöpfe zeigen den Katalog-Typ** (Oberfläche, Mini-Runde, klein, verlustfrei D4)
  ...
  Hinweis: Die Tabelle der Runde 57 (`moveAtUse` in `move-use.ts`) läuft im Haupt-Bundle; sie braucht die Fakten des Schnappschusses (Fähigkeit, Item, Form, Tera, Wetter, Terrain, verdeckter Typ).
...
## Iteration 7b · Rechner: Reste aus Runde 54 und 56

Drei Rechner-Reste: T72 hat der Schiedsrichter der Runde 54 gefunden, T76 die Messung am gezogenen Wurf in Runde 56, T83 die Lesung der Runde 57. Beide berühren Scores nur in Spielen mit einer betroffenen Art oder einem betroffenen Zug; der Beleg ist je eine Zählung. Die zwei Nachbau-Reste der Runde 54 (T74, T70) sind seit Runde 55 erledigt.
...
- [ ] **T83 · Hidden Power geht als nackter Name an den Rechner** (Zufall preisen, Mini-Runde, klein, score-berührend D3)
  ...
  smogtours-gen6ou-648453 Zug 23 (Lesung der Runde 57): Der Spielbericht sagt „Hidden Power Ice kills ~6% of the time“. Die Quote kommt aus @smogon/calc, und der bekommt den Namen des Slots, „Hidden Power“, den er als Normal-Zug mit Stärke 60 liest (Gen 2 bis 5: Stärke 0). Im Spiel traf derselbe Zug Landorus-T für 236 von 319 KP. Betroffen sind `ko-odds.ts` (Gewichte der Singles-Wurzelzellen, die Quote an den Zeilen), `damage-calc.ts` (Schadensvorschau) und `pair/kill-table.ts` (Doubles-Plan). Seit Runde 57 kennt die Statik den verdeckten Typ; die Quote daneben nicht.
...
- [ ] **T79 · Ogerpon trägt seinen festen Tera-Typ und seine Maske** (Sets und Spreads, Mini-Runde, klein, score-berührend D3)
  ...
  Hinweis: Der Bau setzt `teraType` heute nur aus dem Log (`packages/replay-core/src/team-builder.ts:212`); Rückfall auf `species.requiredTeraType`, ebenso das Item auf `species.requiredItem`. Erster Schritt: zählen, wie viele Bank-Stellungen ein Ogerpon oder Terapagos tragen. Die Lesung der Runde 57 fand dieselbe Familie in VGC 2630685175 Zug 4: Der Nachbau baut den Grund-Terapagos mit Teraform Zero und Tera Stellar, das Log zeigt Terapagos-Stellar; Tera Starstorm liest dort selbst im Simulator Normal.
...
- [ ] **T81 · Stärke beim Einsatz übernehmen: Low Kick, Heat Crash, Return, Gyro Ball und Co.** (Re-Fit, Mini-Runde, klein, score-berührend D3, braucht T49 oder T50)
  ...
  smogtours-gen6ou-655336 Zug 6: Bisharp trifft Heatran mit Low Kick für 75 % der KP, die Statik rechnet 0, weil der Katalog Low Kick die Stärke 0 gibt. Dasselbe gilt für Lopunny-Megas Return in allen vier gepinnten Gen-6-Spielen und für Ferrothorns Gyro Ball, in 649664 sein einziger Angriff. Runde 57 hat die Regeln gebaut (34f84b9, Branch `r57-power`): Gewicht (Low Kick, Grass Knot, Heavy Slam, Heat Crash), Freundschaft (Return, Frustration), KP (Flail, Reversal, Crush Grip, Wring Out, Hard Press, dazu Endeavor), Tempo (Gyro Ball, Electro Ball). Der Schiedsrichter stützt sie: 137 falsche Immunitäten fallen auf 0, der Fehler der Gewichts-Züge von 0,398 auf 0,048. Die Bank liest Doubles glücksbereinigt schlechter, +45 bp [+9, +91] gegen den Stand ohne T81, fast ganz aus zwei Turnier-Spielen, in denen Gouging Fire mit Heat Crash steht und verliert; Singles Mitte gewinnt −16 [−28, −5]. Das Muster von T71: Die Statik wird genauer, die Gewichte sind gegen die blinde Statik gefittet.

  Hinweis: 34f84b9 liegt vor dem Zeit-Commit be0d0dd und lässt sich nicht glatt übertragen. Von Hand auf die Spitze bringen: die neuen Fakten (Freundschaft, KP, Gewicht, Tempo) als Getter wie die übrigen, und `landedOrCatalog` muss die Ziel-Fakten weitergeben. Aus dem finalen Review: Low Kick hat in Gen 1 und 2 die feste Stärke 50 (`mods/gen2/moves.js`), Return antwortet in Gen 2 mit `|| null`, und das Tempo rundet in anderer Reihenfolge als der Simulator (Paralyse zuletzt). Cache v54, die Identitäts-Referenz (`fork-identity.json`) auf dem gemeinsamen Stand neu aufnehmen. Die Golden 655336 driftet mit T81 in 6 statt 8 Punkten. Lesung der Runde 57 (`docs/perf/probes/2026-09-23-r57/reading/power-*`): 8 plausibler, 4 gleich, 7 weniger, 3 offen.
...
- [ ] **T89 · Der Bau hält eine Slot-Alternative statt des festen Zugs** (Sets und Spreads, Mini-Runde, klein, score-berührend D3)
  ...
  smogtours-gen6ou-648453: Der gebaute Tornadus-T trägt U-turn, Hidden Power Ice, Hurricane und Heat Wave. Das veröffentlichte Set (Assault Vest Pivot) hat Hurricane, [Heat Wave | Hidden Power Ice], Knock Off und U-turn: Der Bau behält beide Optionen des einen Slots und lässt den festen Knock Off fallen. p1 klickt in 13 Tornadus-Zügen nie Heat Wave, und drei der in Runde 57 bewegten Urteile (Zug 23, 24 und 31) nennen Heat Wave als besten Zug und hängen an ihm (Lesung der Runde 57, Gegenprüfer).
...
- [ ] **T84 · Tera-Optionen werden vor dem Klick bewertet** (Endspiel, Mini-Runde, klein, score-berührend D3)
  ...
  „move terablast terastallize“: Die Kandidaten-Hinweise (`search/hints.ts`) und die K.-o.-Quoten (`ko-odds.ts`) rechnen die Option auf dem Körper vor dem Klick, also Tera Blast als Normal-Zug; in Doubles teilt eine Tera-Variante den Hinweis ihres Grundzugs (`options.ts`). Die Doubles-Beschränkung auf 16 Kombinationen sortiert nach diesen Hinweisen und kann eine Tera-Linie verwerfen, bevor sie gerechnet wird.
...
- [ ] **T87 · Endspiel-Löser und Beweiser halten Züge mit Katalog-Stärke 0 für schadlos** (Endspiel, Mini-Runde, klein, score-berührend D3)
  ...
  `endgame/children.ts` (`damagingPair`) und `endgame/prover.ts` (`critable`, Reihenfolge nach Stärke) lesen die Katalog-Stärke: Ein Zug mit Low Kick, Heat Crash, Gyro Ball, Return oder einer Hidden Power bis Gen 5 bekommt eine Ziehung statt drei, und ein überlebter Treffer daraus trägt keinen Volltreffer-Vorbehalt. Das gilt auf dem einfachen Pfad, wenn der Zellenplan aussteigt.
...
- [ ] Bewegte MCTS-Züge ohne geänderten Wurzelpreis (Runde 57, 648453): Neue Kandidaten-Hinweise ordnen die Baum-Expansion um, und fast jede Zelle eines MCTS-Zugs bewegt sich, obwohl kein Wurzelpreis sich ändert (Zug 23: 26 von 28 Zellen) → handeln, wenn ein Pin nur an so einer Verschiebung hängt; Hebel: Baum-Varianz messen (mehr Bäume oder Iterationen), bevor ein Urteil gelesen wird.
- [ ] Golden 655336 driftet seit vor Runde 57 in acht Punkten (Schlüsselmomente Zug 6, 13 und 26, Fehlzug 26:p2, Reads 13:p1 und 27:p1, Wendepunkt 3 statt 4); T81 nimmt zwei davon weg → beim nächsten Golden-Refresh am User-Gate gesammelt lesen.
