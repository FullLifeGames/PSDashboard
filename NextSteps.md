...
- [ ] Der App-Bau widerspricht 7 von 910 beobachteten Singles-Zugreihenfolgen der Bank und 8 von 267 in Doubles (Runde 53; vor T66 waren es 20, der Bau mit einer Lösung bricht 10). Diese 15 Zeilen bricht jeder Bau, auch der ohne Evidenz: Scarf-Fälle und Maxima auf beiden Seiten, Stoff für T30 → nach jeder Set- oder Löser-Runde neu zählen (`docs/perf/probes/2026-09-20-r53/T66/stage-trace.vt.ts`, Stufe `app`, 17 s); steigt die Zahl, vor der Bank nachsehen. Die Sonde rechnet eine Mega-Entwicklung mit dem Tempo der Grundform.
- [ ] 2658663776 Zug 86 liest nach Runde 53 +0,07 für p1 (vorher +0,99), obwohl p1 acht Züge später aufgibt: Gliscor streckt mit Substitute und Protect Volcanions Flamethrower-PP aus. Die Zugreihenfolge stimmt jetzt; die Engine sieht den PP-Stall nicht → bei einem zweiten Fall ein TODO neben T33.
- [ ] `carryPreSolve` (`team-builder.ts`, bis Runde 53 `carryItemDecisions`) gibt einem Pokémon, das die Volllösung verwirft, den Eintrag der Tempo-Vorlösung zurück. Mit Item-Entscheidung feuert der Zweig in keinem der 139 gemessenen Replays, ohne Item bei 9 Sets in 8 Bank-Replays → wer die App auf eine Lösung zurückbauen will, braucht zuerst einen Fall für den Item-Zweig.
...
- **D1** Kein Push ohne Ansage (Stand 20.09. nach Runde 52: master liegt 71 Commits vor origin/master; origin/master steht auf ba2ce5b, Release 0.7.1). Seit dem 20.09. trägt der Branch `v1` bei origin den master-Stand nach Runde 52 (User-Freigabe 14:50: „auf einen v1 branch, den du pushen darfst“); die Freigabe gilt für `v1`, master selbst bleibt ungepusht.
...
- **D16** `ALIGNMENT_SEEDS[0]` bleibt `'1,2,3,4'`; jede Listen-Änderung ist ein Cache-Version-Event. Runde 49 hat v48 genommen (Merkzettel-Schlüssel), Runde 53 v49 (Übergabe der Tempo-Vorlösung); `r45-threshold` trägt eine v47 und bekäme bei einer Landung die nächste freie Nummer.
