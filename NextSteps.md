# Next Steps (Stand 20.09.2026, nach Runde 53)
...
- **Iterationen** bündeln die TODOs einer Sitzung. Eine Iteration wird beim Start zur Runde mit der nächsten freien Rundennummer: Iteration 1 war Runde 46, Iteration 2 war Runde 47, Iteration 2a war Runde 48, Iteration 2b war Runde 49, Iteration 3 war Runde 50, Iteration 4 war Runde 51, Iteration 4a war Runde 52, Iteration 4d war Runde 53, Iteration 4b wird Runde 54. Score-berührende TODOs derselben Iteration bekommen je ihre eigene Messung (D3), nie eine gemeinsame.
...
- [ ] **T57 · Tera in der Statik: Typen lebend lesen** (Endspiel, Runde, mittel, score-berührend D3)
  ...
  *Erfolg:* Die 18 Stellungen tragen ihren Tera-Körper (Zählung Log gegen Nachbau 0 zu 0), die Statik setzt auf den Bank-Stellungen keine falsche Immunität mehr (heute 50), keine gepoolte Bank-Zeile zeigt Schaden (D3), die Tier-Zählung hält, und die bewegten Doubles-Urteile der vier Spiele ohne Pins sind gelesen. Fällt die Doubles-Zeile, entscheidet das User-Gate zwischen Übernahme mit Re-Fit (T49, T50, T52) und Parken. *Plan T57:* 9 Schritte, 4 offene Entscheidungen.
  ...
  - [x] **8.** (Kette 20.09. 19:40 bis 21:07, Variante ohne STAB bis 21.09. 09:45; `docs/perf/probes/2026-09-20-r54/gates-chain.log`; die STAB-Regel d8eb0a3 zeigt Schaden auf Doubles glücksbereinigt und ist mit c910b91 geparkt, Branch `r54-stab`) Gate-Kette D3 je Zustand: Basis byte-gleich wiederholt, Bank gepaart gegen den Vorzustand und gegen `r53-carry`, Sonden, ein Feedback-Lauf mit Tier-Zählung der bewegten Züge; an der Spitze drei Feedback-Läufe, Suite, e2e, lint, `tsc -b`.
...
## Iteration 7b · Nachbau und Rechner: Reste aus Runde 54

Drei kleine Korrekturen, die das Lesen der Doubles-Urteile, das Review und der Schiedsrichter der Runde 54 gefunden haben. Alle drei berühren Scores nur dort, wo sie greifen; die Bank sieht T70 gar nicht (0 von 46 Doubles-Spielen), der Beleg ist je eine Zählung. T74 steht vorn, weil es heute ein falsches Blunder-Urteil in einem Feedback-Spiel erzeugt.
...
- [ ] **T72 · Der Schadensrechner kennt kosmetische Formen nicht** (Zufall preisen, Mini-Runde, klein, score-berührend D3)
  ...
  Gastrodon-East steht auf dem Feld, und jede Frage an den Schadensrechner kommt leer zurück: @smogon/calc kennt die Art nicht und wirft, `calcSingleDamageRange` fängt das und liefert „-“. 46 der 2529 Paare des Schiedsrichters der Runde 54 sind so leer; mit der Art „Gastrodon“ liest dasselbe Paar 25 bis 30 %. Betroffen ist alles, was den Rechner fragt: die K.-o.-Quoten der Zellen und die Schadensvorschau im Branch-Panel. Ogerpon-Wellspring-Tera steht nach dem Code in derselben Lage (ungemessen).
...
- `r54-stab` (252ade4): Runde 54 mit der STAB-Regel nach den Spielregeln (d8eb0a3), gemessen und geparkt; master trägt die Regel als Commit und nimmt sie mit c910b91 wieder heraus, `git revert c910b91` holt sie zurück. Referenz für T71. Bank-Ordner `.calibration/r54-review` (Spitze mit STAB) und `.calibration/r54-stab` (Schritt allein).
...
- **D1** Kein Push ohne Ansage (Stand 20.09. nach Runde 53: master liegt 75 Commits vor origin/master; origin/master steht auf ba2ce5b, Release 0.7.1). Seit dem 20.09. trägt der Branch `v1` bei origin den master-Stand: nach Runde 52 (User-Freigabe 14:50: „auf einen v1 branch, den du pushen darfst“) und nach Runde 53 (User-Gate 16:22, „3a“). Jede Freigabe gilt für `v1` und für diesen einen Stand; master selbst bleibt ungepusht.
