...
Speed is measured from the replay's own races. Same-turn move order and KOs landed before the victim ever acted (a chosen switch would have resolved first) become hard speed constraints, with directional exclusions that keep only what a modifier strengthens: an attacker outrunning a Tailwind-doubled victim outruns its base speed a fortiori, a paralyzed Pokémon moving first won at a quarter speed, while Trick Room turns and priority races prove nothing. In doubles the parser reads every pair of movers on opposite sides, in the order they acted, as a race. It skips a partner knocked out by a spread move, copied actions (Dancer, Instruct, a bounced status move), a Pursuit fired at a switch, and a slot After You or Quash rearranged. It also drops an order that something other than Speed explains: conditional priority (a Prankster status move, Gale Wings at full HP, a Triage heal, Grassy Glide on its terrain), a Quick Claw, Quick Draw, or Custap Berry that fired in the log, a weather or terrain ability the mover's species can carry (Swift Swim in rain, Chlorophyll in sun, Sand Rush, Slush Rush, Surge Surfer) unless the protocol revealed another ability, an Unburden holder that lost its item, and a Stall or Mycelium Might user moving last.
...
Solved spreads flow into every simulator team and show in the stats panel with the tag "fitted" (detail "fits observed damage"), so branches stop KOing Pokémon that survived the same hit in the replay. Evidence that cannot measure keeps the prior: a knock-out line is only a lower bound, so an attacker seen only in knock-outs keeps its guessed offense investment, and a move order no legal spread of that Pokémon can reproduce keeps its guessed Speed; those kept stats give way last in the EV budget, never to a bulk hypothesis. With no usage entry at all, the default spread invests the offense side the revealed attacks agree on.

The solver reads a move order the mover cannot reach at full Speed as a Choice Scarf. Think of a runner who arrives faster than any legal build allows: before the solver blames the item, it checks that the opponent did not dawdle. It measures the mover against every common spread of the opponent's species (usage spreads with at least a 5% share, thinned to the Speed camp when one camp holds 80% of them), or against the opponent's real spread when a sheet, the protocol, or an edit fixed it. The mover gets the Scarf only when even the slowest plausible opponent stays out of reach, the Scarf would close the gap, and the species runs Speed-invested spreads at all (a Ho-Oh does not). The same rule runs backwards: when a guessed Scarf would make its holder faster than an observed order allows, the solver drops it and the build takes the next usage item. The solver leaves known items, sheet sets, and Choice items the protocol ruled out alone. The panel shows an inferred Scarf with the tag "inferred" (detail "moved first, Choice Scarf inferred") and a replaced one as a guess with the detail "moved second, Choice Scarf ruled out". The sensitivity probes skip an inferred Scarf, since its alternatives would contradict the evidence, and keep a dropped Scarf out of a replaced item's alternatives.
...
- guessed: usage statistics (a spread fitted to observed damage shows as "fitted", a Choice Scarf the move order proved as "inferred"),
- manual: your edits.
...
- MCTS: a DUCT Monte-Carlo tree search; a forced switch after a knock-out is a decision node of its own inside the tree (the replaced side picks from its bench with lookahead, the other side waits), and in the first two plies a cell whose pair carries a priced roll is a chance node with one child per outcome class (the rolls drawn on demand through a scripted simulator PRNG, the cell valued as the weighted blend; doubles pairs group three fixed seeds by who fell), while deeper cells fix one outcome per tree as before, while the root matrix, the verify sampler, play-outs and the prover keep resolving replacements greedily by the static pick,
...
Where the board is small (at most three living bodies, or the last pair) or the decided sweep names a side, a forced-win prover runs after the root search. It plays the favored side's best moves against every reply in the simulator, outcome class by outcome class, until the game ends, and reports the proven mass: the share of the outcome classes along which the side wins against everything. A proof at or above one half lifts the score to that mass, with the rest at the open branches' static value; the matrix, the rankings, and the interval stay as searched, and the engine's own score is kept next to the bar. At or above 0.9 the turn card says so in words: "X wins in N against every reply", with "if the 95% Fire Fang lands" or "if the 90% Fire Fang knocks out" when one roll stays open, "barring a crit" when a defender survived a hit the class model never priced for a crit, and "on the sampled rolls" for doubles and for cells whose three draws disagreed. Finding the line is cheap, refuting every reply is not, so a greedy probe runs first: one line per candidate move against the reply that leaves the opponent the most HP, a few cells per turn, and only a line that ends in a win within five turns starts the full search. The prover is budgeted by matrix cells (100 per position, the probe's cells reused) and never by wall clock, so the same position always proves the same way. A class the base seeds never show is drawn on demand (the simulator's accuracy, crit and damage rolls scripted for that one move, the log confirming the class); a class the simulator refuses stays open and lowers the mass instead of failing the proof, unless a drawn class is at least as bad for the side (the opponent's miss inherits from their hit, the side's knock-out from its survived hit). On boards above three bodies the prover waits for the search to read the game at 0.6 or beyond; the graph sweep's sketch pass skips it, because the full pass replaces every sketch.
...
- `EVAL_ENDGAME_TRUTH=1`: the endgame truth bench. A solver under `packages/eval-engine/src/endgame/` computes the exact value of small endgames (at most three living bodies, singles and doubles) under best play by both sides: every state is a turn boundary, its value the equilibrium of the choice-pair matrix over the children's values, recursively to the game's end, memoized on the serialized board. Chance is priced per cell as outcome classes with analytic weights and one representative child per class; a singles speed tie splits into two order classes. Three caps keep it finite (30 turns, 20000 states, 120 s); a result is `exact` only when no cap, no unpriced chance, and no loop was touched, otherwise it carries the flags `capped`, `unpriced`, or `loop` and the search's leaf value stands in. The bench runs the static, the last-pair sweep variant, matrix depth 1 to 3, and MCTS against the exact value on the bank's exported endgame positions (`EVAL_CALIBRATION_POSITIONS=<dir>` on the calibration sweep) and on the synthetic fixtures in `regression/endgame-fixtures.ts`; `node scripts/endgame-truth-table.mjs` renders the dump as coverage, per-estimator Brier and sign agreement on the exactly solved positions, and a per-position table. The solver is an instrument, not a production path.

The calibration sweep prints two more lines under the pooled K since round 34: `hq:` restricts the bank to tournament replays (smogtours) and ladder games rated 1700 or above, the pre-registration tranche for later score verdicts, and `luck-adjusted:` drops every position where the protocol shows a crit taken, an own miss, or a paralysis, freeze, or flinch skip against the side the score favored, with the count of exclusions. `scripts/paired-calibration.mjs` and `scripts/subset-calibration.mjs` take `--quality hq|std`.
