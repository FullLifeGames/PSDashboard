...
export const FEEDBACK_CORPUS: FeedbackItem[] = [
  ...
  },
  {
    ...
    essence: 'Misplay verdict against BKC with unusable reasoning; if the play works, the opponent must sacrifice into Lopunny. Missing principle: an opposing mon with NO remaining switch-ins makes any successful switch into it profitable (even via U-turn). (Re-pinned 2026-08-15: the mistake verdict evaporated once the silently lost neighbor turns 14-16 evaluate — toward the expert; the no-switch-ins principle stays open.) (Round 13 2026-08-25, user-gated: the principle LANDED ON THE TURN — the search root computes which living mons the other side has no live race answer to (a benched answer pays the switch-in entry toll: hazard-adjusted arrival minus one free hit — the first landing measured standing pair races and saw full-HP Weavile as an answer, while t14\'s actual Weavile sacrifice is the expert\'s proof of the entry economy), and t13 now renders "Bisharp has no live answer on the other side — any turn that brings it in cleanly turns profit, and the opponent can only sacrifice into it" for the ENGINE\'S OWN recommendation (best "→ Bisharp"). REMAINING gap: the sentence speaks for Bisharp, not for BKC\'s actually played Lopunny switch — the race model still finds a toll-surviving answer to Lopunny in this position (in 653785 t14/t15 the same signal does name Lopunny-Mega).)',
    observed: { side: 'p2', tier: 'none', attribution: ['quiet'], summaryIncludes: ['no live answer on the other side'] },
