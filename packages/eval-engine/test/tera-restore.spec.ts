...
describe('Tera marker after a reconstruction-only faint (round 54)', () => {
  ...
  // leave half a Tera. The loss stays on those bodies.
  test('Ogerpon is left alone', async () => {
  ...
  test('a side with an Illusion holder is left alone (the protocol names the disguise)', async () => {
    const teams = {
    ...
    const { battle, snapshot } = await rebuilt('gen9ou', log, teams);
    const snorlax = faintInTheRebuild(battle, 'Snorlax');
