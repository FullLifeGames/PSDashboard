...
describe('doubles combined matching', () => {
  ...
  test('matches per-slot moves with targets, spreads, tera, and switches', async () => {
    const { matchPlayedSide } = await import('../src/analysis');
    const match = (slots: import('../src/played').PlayedTurn['p1Slots']) =>
...
describe('choice diffing (the condensed why)', () => {
  ...
  test('a skipped gimmick names the gimmick alone', async () => {
    const { diffChoices } = await import('../src/analysis');
    expect(diffChoices(
...
describe('narrative signals (round 5)', () => {
  ...
  // Round 54: the guard reads the DEFENDER's Tera type, per side. Only p1 has
  // terastallized (Gholdengo to Flying): p2's Earthquake is null because of
  // it, and p1's Shadow Ball into the untouched Snorlax stays null as before.
  test('the null guard reads the defender\'s Tera type from the other side', () => {
    ...
    expect(analysis.p2.bestNull?.reason).toContain('immune to Ground-type moves');
    expect(analysis.p1.bestNull?.reason).toContain('immune to Ghost-type moves');
  });
