...
The layering is enforced, not just described. `eslint.zones.mjs` turns the target order (app, then the engine under `src/lib/eval` with the reconstruction libs, then the replay core: `src/types`, `src/lib/ids.ts`, `src/lib/calc-field.ts`) into `no-restricted-imports` errors: nothing under `src/lib` or `src/types` imports hooks, components, App, or workers; the engine imports only replay-core modules from outside its folder; components use the branch and evaluation facades. The duplicates that had grown across the layers are gone with it: one `toId` in `src/lib/ids.ts` (28 copies under nine names before), one stat-stage multiplier (`stat-stages.ts`; the accuracy-stage formula in `ko-odds.ts` is a different table and now says so), one living-bodies filter, one side-index helper, one side-invariant restore shared by the forward model and the correction pass, and the sim-to-calc field tables in `calc-field.ts` (the spread fit, a replay-core module, reads them too, so they sit below the engine). `knip` keeps the export surface honest (its report is empty; internal helpers are not exported), and a jscpd census merged the copy-paste blocks a helper reads naturally; the six kept clones are listed in the round ledger with their reasons.
