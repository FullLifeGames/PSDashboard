...
**Hax alignment (per-turn seed search).** The replayed choices are the real game's, but the simulator rolls its own RNG — a phantom crit or miss the real game never had can contaminate a turn or even end the battle early, and boundary corrections cannot revive an ended battle. So before each block commits, the reconstruction serializes a checkpoint of the live battle, trials the block's choices under a pinned 16-seed candidate list (`src/lib/hax-alignment.ts` scores each trial's emitted events against the protocol block: game end and the faint set compare first, then soft counts over misses/crits/secondaries/hit counts/`|cant|`s/confusion self-hits/move counts plus a move-order signal), and calls `battle.resetRNG` with the best candidate — deterministically (ordered trials, early exit on a perfect match, ties keep the earlier seed), reseeding every turn so one turn's RNG consumption never shifts the next turn's rolls. The trial runner is `trialAdvanceLog` in the forward model (a fork-based sibling of `advancePosition` that answers forced switches from the protocol's replacement species); the runtime records each block's chosen seed plus the truly emitted block's score, and the sweep surfaces those records on `window.__psDebug.haxAlignment`, a premature-end notice, and the drift report's meta section. Damage magnitude is deliberately unscored — boundary snapshots already pin HP, and matching rolls would reward wrongly guessed spreads.
